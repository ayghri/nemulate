...
def interpolate_field_1d(long, lat, values, target_long, target_lat):
    # long = np.arange(-359, 360, 2) / 2
    # lat = np.arange(-179, 180, 2) / 2
    mesh_grid = (
        np.stack(np.meshgrid(long, lat)).transpose((1, 2, 0)).reshape(-1, 2)
    )
    target_mesh_grid = (
        np.stack(np.meshgrid(target_long, target_lat))
        .transpose((1, 2, 0))
        .reshape(-1, 2)
    )
    return LinearNDInterpolator(mesh_grid, values)(target_mesh_grid)
